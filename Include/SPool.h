...
#define __TRIVIAL -1
	...
	public:
		...
		// To get actual count subtract by free-item-count
		int GetSize ( void );
	...
	template <class T>
	T*	SPool<T>::GetItemByUID ( SPoolIndex iIndex )
	{
	...
	template <class T>
	T*	SPool<T>::GetItemByIndirectionIndex ( UINT iIndex )
	{
	...
	template <class T>
	typename SPool<T>::SPoolObject* SPool<T>::GetPoolObjectByIndirectionIndex( UINT iIndex )
	{
		if( __ISSET( pMainList ) && __ISSET( pIndirections ) && nSize > 0 && (int)iIndex < nSize
			&& pIndirections[(int)iIndex] != __TRIVIAL )
		{
	...
	template <class T>
	T*	SPool<T>::DeleteItem ( SPoolIndex iIndex )
	{
	...
	template <class T>
	int		SPool<T>::GetSize( void ) { return nSize; };
	...
	template <class T>
	int		SPool<T>::GetFreeCount( void ) { return nFrees; };
