...
namespace SpeedPoint
{
	...
	// Notice: This Physics Engine used physics-specific implementations of the abstract classes ISolid and ISolidSystem.
	...
	public:
		...
		/*
		SResult HandlePhysics(SPhysicalSolidSystem* pPhyISolidSystem)
		{
			unsigned int nPhyISolids = pPhyISolidSystem->GetSolidCount();

			// Go through all solids found in this solid system and let it interact with all other solids		
			for (unsigned int iSolid1 = 0; iSolid1 < (nPhyISolids - 1); ++iSolid1)	// -1 cause we dont want the last solid to interact with itself
			{
				SPhysicalSolid* pSolid1 = (SPhysicalSolid*)pSolidSystem->GetSolid(iSolid);
				for (unsigned int iSolid2 = (iSolid1 + 1); iSolid2 < nPhyISolids; ++iSolid2)
				{
					...
					float fElapsedTime = pPhyISolidSystem->GetEngine()->GetElapsedTimeLastFrame();
