...
void CSMBLoader::ParseError(const char* msg) const
{
	...
	std::cout << "[SMBLoader] Error @" << m_LineCounter << ": " << msg << std::endl;
...
void CSMBLoader::ReadSMBFile(const char* filename, std::vector<SSMBMaterial>& materials)
{
	...
	while (true)
	{
		...
		{
			pChunk = new CSMBTestMaterialChunk();
		}
		// else ...
		...
		while (true)
		{
			...
			if (result == eSMBREAD_OK)
			{
				...
				if (!pChunk->SetProperty(property_name, property_value))
					ParseError("Failed set property");
			}
		}
...
bool CSMBWriter::Prepare(const char* filename)
{
	m_Stream.open(filename, ofstream::trunc | ofstream::out);
...
CSMBTestMaterialChunk::CSMBTestMaterialChunk()
{
}

bool CSMBTestMaterialChunk::CheckParameters(const vector<string>& params) const
{
	return (params.size() >= 2);
}

void CSMBTestMaterialChunk::Create(const string& name)
{
	std::cout << "CSMBTestMaterialChunk::Create('" << name << "')" << std::endl;
}

bool CSMBTestMaterialChunk::SetProperty(const string& name, const string& value)
{
	if (name == "texture_map")
	{
		string texMap;
		if (!CSMBLoader::ParseString(value, texMap))
			return false;

		texMap = CSMBLoader::TrimString(texMap);

		std::cout << "texture_map: '" << texMap << "'" << std::endl;
	}
	else if (name == "normal_map")
	{
		string normalMap;
		if (!CSMBLoader::ParseString(value, normalMap))
			return false;

		normalMap = CSMBLoader::TrimString(normalMap);
...
void CSMBTestMaterialChunk::Write(CSMBWriter* pWriter, const SSMBMaterial& material)
{
	if (!IS_VALID_PTR(pWriter))
		return;

	pWriter->OpenChunk("mat 'testmaterial'");

	pWriter->WritePropertyLine("texture_map", "test-texture-map");
	pWriter->WritePropertyLine("normal_map", "test-normal-map");
	pWriter->WritePropertyLine("roughness_map", "test-roughness-map");
	pWriter->WritePropertyLine("emissive", 0.2f, 0.2f, 0.8f);
	pWriter->WritePropertyLine("illum_model", 0U);
...
void CSMBMaterialChunk::Create(const string& name)
{
...
bool CSMBMaterialChunk::SetProperty(const string& name, const string& value)
{
	if (name == "texture_map")
	{
		...
		m_Material.textureMap = CSMBLoader::TrimString(texMap);
	}
	else if (name == "normal_map")
	{
		string normalMap;
		...
		m_Material.normalMap = CSMBLoader::TrimString(normalMap);
	}
	else
	{
		return false;
