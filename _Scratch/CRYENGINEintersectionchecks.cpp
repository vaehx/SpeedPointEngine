...
// Assumes pcyl->axis is normalized!
int tri_cylinder_intersection(const triangle* ptri, const cylinder* pcyl, prim_inters* pinters)
{
	...
	// Find LAST triangle point that is OUTSIDE of the cylinder
	for (i = 0,iStart = -1; i < 3; i++)	
	{		
		// center to triangle point
		pc                      = ptri->pt[i] - pcyl->center;
		
		// pcyl->hh ... half height
		// isneg(a - b) == b > a
		// bOutside =  dist(point, capsule) > rad  ||  point outside of planes  
		// bOutside = (len2(cross(pc, pcyl->axis)) > r2) || (abs(dot(pc, pcyl->axis)) > pcyl->hh)
		bOutside                = isneg(r2 - (pc ^ pcyl->axis).len2()) | isneg(pcyl->hh - fabs_tpl(pc * pcyl->axis));

		// if (bOutside) iStart = i 		
		(iStart &= ~-bOutside) |= i & - bOutside;		
	}

	// If there was no triangle point outside of the cylinder, iStart is left -1
...
int tri_capsule_intersection(const triangle* ptri, const capsule* pcaps, prim_inters* pinters)
{
	...
	// Find last triangle point that is OUTSIDE of the capsule
	for (i = 0,iStart = -1; i < 3; i++)
	{
		// Vector from capsule center to triangle point
		...
		//   a % b  =  a & (b - 1)
		//
		// Check if integer is even/uneven
		//   even:  ((i & 1) == 0)
		//
		// Absolute value:
		//   #define abs(i) (i < 0 ? -i : i);
		(iStart &= ~-bOutside) |= i & -bOutside;	// <=> if (bOutside) iStart = i;
	}
...
int ray_tri_intersection(const ray* __restrict pray, const triangle* __restrict ptri, prim_inters* __restrict pinters)
{
	...
	{
		const float fSign   = (float)__fsel(fDotDir0, 1.0f, -1.0f);
		...
		for (i = -1; i <= 1; i += 2)
		{
			...
			else
			{
				...
				if (cube(p) * (1.0f / 27) + sqr(q) > 0)
				{
					...
					Vec2i sg2d(sgnnz(c2d.x),sgnnz(c2d.y));
					Vec2  pt2d,kpt2d,n2d,kn2d;
