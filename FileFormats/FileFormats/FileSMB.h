...
// Whenever adding to an existing SMB File, if a chunk with the same name already exists, the
// new chunk OVERIDES the existing chunk completely. 
...
public:
	...
	// append: Set to true to append to the end of the file if it exists. If set to false, file contents will be lost.
	bool Prepare(const char* filename, bool append = false);
	void Close();

	void WriteSectionComment(const string& section);
